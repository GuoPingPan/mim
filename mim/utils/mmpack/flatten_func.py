...
class TopClassNodeInfo:
    ...
    super_cls_name: str
    end_idx: int
    sub_func: Dict[str, ast.FunctionDef] = {}
    sub_assign: Dict[str, ast.Assign] = {}
...
class TopAstInfo:
    ...
    importfrom_dict: Dict[str, List[ast.ImportFrom, List[str]]] = {}
    importfrom_asname_dict: Dict[str, ast.ImportFrom] = {}
    import_list: List[str] = []
    try_list: List[ast.Try] = []
    if_list: List[ast.If] = []
    assign_list: List[ast.Assign] = []
...
class ExtraSuperAstInfo:
    ...
    """
    used_module: Dict[ast.AST, Set[str]] = defaultdict(set)
    extra_import: List[ast.alias] = []
    extra_importfrom: Dict[ast.ImportFrom, List[ast.alias]] = {}
...
class NeededNodeInfo:
    ...
    """
    need_importfrom_nodes: Set[ast.ImportFrom] = set()
    need_import_alias_asname: Set[ast.alias] = set()
    need_import_alias: Set[ast.alias] = set()
    ...
    for node in super_ast_tree.body:
        ...
        # ast.Module -> ast.ClassDef
            ...
            for sub_mode in ast.walk(top_cls_node):
                ...
                if isinstance(sub_mode, ast.Attribute) \
                        and hasattr(sub_mode, 'value') \
                        and isinstance(sub_mode.value, ast.Call) \
                        and isinstance(sub_mode.value.func, ast.Name) \
                        and sub_mode.value.func.id == 'super':  # noqa: E501
                    """
                    ...
                    for super_cls_sub_node in node.body:
                        if isinstance(
                            ...
                            super_func.append(sub_mode.attr)
                            sub_mode.value = \
                                sub_mode.value.func
            ...
            for super_cls_sub_node in node.body:
                ...
                if isinstance(super_cls_sub_node, ast.FunctionDef):
                    ...
                    # directly imported
                    elif super_cls_sub_node.name not in top_ast_info.class_dict_top.sub_func:
                        func_need_append.append(super_cls_sub_node)
...
def flatten_inheritance_chain(top_ast_tree: ast.Module, obj_cls: type):
    ...
    print_log(f'*[inheritance_chain]: {inheritance_chain}\n',
              logger='export',
              level=logging.INFO)
    ...
    for cls in inheritance_chain:
        ...
        used_module_dict_super, extra_import_list_super, \
            extra_import_from_dict_super = collect_needed_node_from_super(
